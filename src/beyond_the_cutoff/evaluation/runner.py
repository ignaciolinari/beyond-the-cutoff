...
def evaluate_pregenerated_responses(
    *,
    responses_path: Path,
    judge_prompt_path: Path,
    judge_inference_cfg: InferenceConfig,
    ...
    """Evaluate pre-generated model responses using the judge.

    This is Phase 2 of two-phase evaluation. It takes responses that were
    previously generated by generate_responses.py and evaluates them.

    Args:
        responses_path: Path to JSONL file with pre-generated responses
        judge_prompt_path: Path to judge prompt config
        judge_inference_cfg: Judge inference configuration
        model_label: Label for this evaluation run
        output_path: Path to save metrics summary
        details_output_path: Path to save detailed results
        metadata_output_path: Path to save run metadata
        limit: Maximum number of responses to evaluate
        max_retries: Max retries for judge calls
    ...
    if checkpoint_path is None and output_path:
        ...
        checkpoint_path = Path("evaluation/results") / model_label / "judge_checkpoint.json"
    ...
    if resume_from_checkpoint and checkpoint_path:
        ...
        if checkpoint_result:
            ...
            checkpoint_loaded = True
            print(
                f"[info] Resuming from checkpoint: {len(evaluated_task_ids)} already evaluated",
                file=sys.stderr,
            )
    ...
    # Determine prompt mode from first response
    prompt_mode = "rag"
    with responses_path.open("r", encoding="utf-8") as f:
        first_line = f.readline()
        if first_line.strip():
            first_record = json.loads(first_line)
            prompt_mode = first_record.get("prompt_mode", "rag")

    print("=" * 80, file=sys.stderr)
    print("[info] Starting judge evaluation of pre-generated responses", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
    print(f"[info] Responses file: {responses_path}", file=sys.stderr)
    print(f"[info] Model label: {model_label}", file=sys.stderr)
    print(f"[info] Prompt mode: {prompt_mode}", file=sys.stderr)
    print(f"[info] Judge: {judge_prompt.name} ({judge_inference_cfg.model})", file=sys.stderr)
    print(f"[info] Total responses: {total_responses}", file=sys.stderr)
    if checkpoint_loaded:
        print(f"[info] Already evaluated: {len(evaluated_task_ids)}", file=sys.stderr)
    print("-" * 80, file=sys.stderr)
    ...
    with responses_path.open("r", encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            task_id = str(record.get("task_id", f"line_{idx}"))
            ...
            # If generation had an error, skip judging
            if generation_error:
                record_errors["generation"] = generation_error
                error_categories["generation"] = record.get("error_category", "unknown")

            # Track empty responses
            has_empty_response = not response_text.strip()
            if has_empty_response and not generation_error:
                record_errors.setdefault("generation", "Empty response received")
                error_categories.setdefault("generation", "validation")

            # Compute citation metrics
            has_contexts = prompt_mode == "rag" and len(contexts) > 0
            ...
            if generation_error or has_empty_response:
                record_errors.setdefault("judge", "Skipped due to generation failure")
                error_categories.setdefault("judge", "validation")
            else:
                judge_start = time.time()
            ...
            if (
                ...
                _save_checkpoint(
                    ...
                    evaluated_task_ids,
                    model_label=model_label,
                    dataset_path=responses_path,
                )

            # Progress update
            elapsed = time.time() - evaluation_start_time
            rate = examples_processed / elapsed if elapsed > 0 else 0
            remaining = (total_responses - len(evaluated_task_ids)) / rate if rate > 0 else 0

            print(
                f"\r[{len(evaluated_task_ids)}/{total_responses}] "
    ...
    summary = summarise_scores(score_rows)
    summary["evaluation_time_seconds"] = total_time
    summary["total_examples"] = total_responses
    summary["evaluated_task_ids"] = list(evaluated_task_ids)
    summary["model_label"] = model_label
    summary["prompt_mode"] = prompt_mode
    summary["judge_name"] = judge_prompt.name
    summary["judge_model"] = judge_inference_cfg.model
    summary["examples_evaluated"] = len(score_rows)
    summary["examples_with_errors"] = sum(1 for row in score_rows if row.get("errors"))
    summary["timing"] = _compute_timing_stats(score_rows)

    # Resolve output paths
    if output_path is None:
        output_path = Path("evaluation/results") / model_label / "metrics.json"
    if details_output_path is None:
        details_output_path = output_path.with_name("details.jsonl")

    metadata_path = _resolve_metadata_path(metadata_output_path, output_path, model_label)
    ...
    # Save metadata (simple append to JSONL)
    metadata_record = {
        "model_label": model_label,
    ...
    print("-" * 80, file=sys.stderr)
    print(f"[info] Judge evaluation complete for {model_label}", file=sys.stderr)
